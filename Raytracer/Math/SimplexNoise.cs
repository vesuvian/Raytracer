...
namespace Raytracer.Math
{
    ...
    {
	    private static readonly byte[] s_PermOriginal = {
	    ...
	    public int Seed
	    {
		    ...
		    set
		    {
			    if (value == 0)
			    {
				    m_Perm = new byte[s_PermOriginal.Length];
				    s_PermOriginal.CopyTo(m_Perm, 0);
			    }
			    else
			    {
				    m_Perm = new byte[512];
	    ...
	    public float Calc(float x, float scale)
        {
            return Generate(x * scale);
        }

        public float Calc(float x, float y, float scale)
        {
            return Generate(x * scale, y * scale);
        }

        public float Calc(float x, float y, float z, float scale)
        {
