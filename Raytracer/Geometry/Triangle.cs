		...
		private static IEnumerable<Vertex> SutherlandHodgmanPolygonClip(IEnumerable<Vertex> polygon, Aabb aabb)
		{
			...
			foreach (Plane clippingPlane in aabb.Planes)
			{
				...
				for (int index = 0; index < input.Count; index++)
				{
					...
					Vertex previous = input[MathUtils.ModPositive(index - 1, input.Count)];
					Vertex current = input[index];

					bool previousInFront = !clippingPlane.IsBehind(previous.Position);
					...
					float delta = (position - previous.Position).Length() /
					              (current.Position - previous.Position).Length();
					Vertex intersection = GetInterpolated(previous, current, delta);
